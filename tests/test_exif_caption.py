...
from ideogram_captioner.schema import has_obj_bbox_annotation, normalize_caption
...
class ExifCaptionTests(unittest.TestCase):
    def test_has_obj_bbox_annotation_requires_obj_type(self):
        self.assertTrue(has_obj_bbox_annotation(CAPTION_WITH_OBJ_BBOX))
        self.assertFalse(has_obj_bbox_annotation(CAPTION_TEXT_ONLY_BBOX))
        self.assertFalse(has_obj_bbox_annotation({"compositional_deconstruction": {"elements": []}}))
    ...
    def test_imports_valid_obj_bbox_json_from_largest_workflow_text(self):
        with tempfile.TemporaryDirectory() as temp:
            image_path = Path(temp) / "sample.png"
            ...
            self.assertIn("Imported caption with bbox", message or "")
            self.assertEqual(
    ...
    def test_ignores_workflow_without_obj_bbox_json(self):
        with tempfile.TemporaryDirectory() as temp:
            image_path = Path(temp) / "sample.png"
...
class StoreExifImportTests(unittest.TestCase):
    def test_missing_caption_file_imports_from_exif_workflow(self):
        with tempfile.TemporaryDirectory() as temp:
            folder = Path(temp)
    ...
    def test_existing_caption_file_takes_priority_over_exif(self):
        with tempfile.TemporaryDirectory() as temp:
            folder = Path(temp)
            image_path = folder / "sample.png"
            _save_png_with_comfy_prompt(image_path, [json.dumps(CAPTION_WITH_OBJ_BBOX)])
